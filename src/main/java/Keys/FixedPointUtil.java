...
class FixedPointUtil {
    private static final String PRECOMP_NAME = "bc_fixed_point";
      ...
      static FixedPointPreCompInfo precompute(final ECPoint p) {
        ...
        HashMap<String, FixedPointPreCompInfo> table = extracted(p);
        if (table == null)
        ...
        for (int i = 0; i < lookupTable.length; ++i)
			if (lookupTable[i] != null && lookupTable[i].isNormalized()) {
				zs[count] = (lookupTable[i].zs.length <= 0) ? null : lookupTable[i].zs[0];
				indices[count++] = i;
			}
	...
	private static HashMap extracted(final ECPoint p) {
		return p.preCompTable;
	}
}
