...
public class PublicFromPrivate {
    public String getPublicFromPrivate(String privatekey) throws Exception {
		...
		BigInteger privKey;
		privKey = new BigInteger(cleanValue, 16);
		X9ECParametersHolder holder;
		holder = CustomNamedCurves.nameToCurve.get("secp256k1");
		X9ECParametersHolder holder1;
		holder1 = CustomNamedCurves.nameToCurve.get("secp256k1");
		ECPoint point;
		point = new FixedPointCombMultiplier().multiply(new ECDomainParameters(
				Objects.requireNonNull(holder1 == null ? null : holder1.getParameters()).getCurve(),
				(holder == null ? null : holder.getParameters()).getG()).G, privKey);
		byte[] encoded;
		encoded = point.getEncoded(false);
		int newLength;
		newLength = encoded.length - 1;
		byte[] copy;
		copy = new byte[newLength];
		System.arraycopy(encoded, 1, copy, 0, Math.min(encoded.length - 1, newLength));
		BigInteger value;
		value = new BigInteger(1, copy);
		String publicKeyNoPrefix;
		publicKeyNoPrefix = null;
		String input;
		String result10;
		result10 = value.toString(16);
		input = "0x" + result10;
		if (!(input.length() == 0)) {
			if (input.length() > 1) {
				if (input.charAt(0) == '0') {
		...
		int len9;
		len9 = Objects.requireNonNull(cleanInput9).length();
		byte[] data9;
		int startIdx9;
		data9 = new byte[len9 / 2];
		startIdx9 = 0;
		for (int i10 = startIdx9; i10 < len9; i10 += 2) {
			data9[(i10 + 1) / 2] = (byte) ((Character.digit(cleanInput9.charAt(i10), 16) << 4)
		...
		int len7;
		len7 = Objects.requireNonNull(cleanInput7).length();
		byte[] data7;
		int startIdx7;
		data7 = new byte[len7 / 2];
		startIdx7 = 0;
		for (int i8 = startIdx7; i8 < len7; i8 += 2) {
			data7[(i8 + 1) / 2] = (byte) ((Character.digit(cleanInput7.charAt(i8), 16) << 4)
		...
		byte[] result2;
		result2 = kecc7.digest();
		byte[] bytes3;
		String cleanInput6;
		cleanInput6 = publicKeyNoPrefix;
		int len6;
		len6 = Objects.requireNonNull(cleanInput6).length();
		byte[] data6;
		int startIdx6;
		data6 = new byte[len6 / 2];
		startIdx6 = 0;
		for (int i7 = startIdx6; i7 < len6; i7 += 2) {
			data6[(i7 + 1) / 2] = (byte) ((Character.digit(cleanInput6.charAt(i7), 16) << 4)
		...
		byte[] result6;
		result6 = kecc3.digest();
		byte[] bytes7;
		String cleanInput2;
		cleanInput2 = publicKeyNoPrefix;
		int len2;
		len2 = Objects.requireNonNull(cleanInput2).length();
		byte[] data2;
		int startIdx2;
		data2 = new byte[len2 / 2];
		startIdx2 = 0;
		for (int i3 = startIdx2; i3 < len2; i3 += 2) {
			data2[(i3 + 1) / 2] = (byte) ((Character.digit(cleanInput2.charAt(i3), 16) << 4)
